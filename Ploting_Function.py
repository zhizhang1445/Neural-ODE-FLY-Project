...
def get_data_func(func, size = 100, ma = None, domain = [-1, 1]):
    X = np.linspace(domain[0], domain[1], size)
    x, y = np.meshgrid(X, X)
    dXdt = func(torch.Tensor(np.stack([x, y], -1).reshape(size*size, 2))).cpu().detach().numpy()
    dXdt = dXdt.reshape(size,size,2)
    if ma is not None:
        dxdt = np.ma.masked_array(dXdt[:,:,0], mask=ma)
        dydt = np.ma.masked_array(dXdt[:,:,1], mask=ma)
    else:
        dxdt = dXdt[:,:,0]
        dydt = dXdt[:,:,1]
    return (x,y), dxdt, dydt

def get_data_V(func, size = 100, ma = None, num = None, flip = False, domain = [-1, 1]):
    X = np.linspace(domain[0], domain[1], size)
    x, y = np.meshgrid(X, X)

    if num is not None:
        a = func.potential(num, torch.Tensor(np.stack([x, y], -1
        ).reshape(size * size, 2)))
        h = a.reshape(size,size).detach().numpy()
    else:
        a = func.total_V(torch.Tensor(np.stack([x, y], -1
        ).reshape(size * size, 2)))
...
def get_data_eig(func, size = 20, erode_size = 2, eig_type = 'vector', ma = None, eig_scale = True, domain = [-1, 1]):
